<template>
    <div>
        ...
        <vs-card v-for="image in images" :key="image.id" id="images-list" style="padding: 0px">
            <ReImage v-on:delete="deleteImage(image.id)" v-on:updatePrimary="updatePrimaryImage(image.id)" :imagePath="/[^/]*$/.exec(image.fileName)[0]" :isBusiness="true" :primaryImagePath="primaryImagePath" class="title-image"></ReImage>
        </vs-card>
